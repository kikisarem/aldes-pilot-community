...
static void dns_server_process(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *src_addr, u16_t src_port) {
    ...
    _Alignas(4) uint8_t dns_msg[MAX_DNS_MSG_SIZE];
    ...
    while(question_ptr < question_ptr_end) {
        if (*question_ptr == 0) {
            ...
            if (label_len > 63 || (size_t)(question_ptr_end - question_ptr) < (size_t)label_len) {
    ...
    // Bound both the incoming question and the 16-byte answer before writing.
    if (question_ptr >= question_ptr_end || question_ptr[-1] != 0 ||
        (size_t)(question_ptr_end - question_ptr) < 4 ||
        (size_t)(question_ptr - dns_msg) + 4 + 16 > sizeof(dns_msg) ||
        question_ptr[0] != 0 || question_ptr[1] != 1 ||
        question_ptr[2] != 0 || question_ptr[3] != 1) goto ignore_request;
    question_ptr += 4;
